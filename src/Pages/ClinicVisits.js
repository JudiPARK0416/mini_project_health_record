...
export const ClinicVisitsContainer = styled.main`
  display: flex;
  flex-direction: column;
  width: 1200px;
`;
export const ClinicVisitsMain = styled.div`
...
const ClinicVisits = ({ todoList, setTodoList, myInfo }) => {
  return (
    <>
      <ClinicVisitsContainer>
        <Header text={"기타내원"}num={todoList.filter(todo => todo.classification==="기타내원").length}/>
        <ClinicVisitsMain>
          <FormContainer>
            <AddBtn>+</AddBtn>
            <InputContentSection>
          ...
          <div>
            {todoList.filter(todo => todo.classification==="기타내원").map((todo, idx)=>
